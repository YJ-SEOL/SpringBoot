  ...
  <div class="container mt-5 pt-5">
    ...
    <table class="table">
      ...
      <tr>
        ...
        <td>{{ complain.target }}</td>
        <td>{{ complain.writer }}</td>
        <td>
          <a :href="'/editcomplain/' + complain.id" class="btn btn-primary">
            Edit
          </a>
        </td>
      </tr>
  ...
  methods: {
    ...
    retrieveComplains() {
      ...
      ComplainDataService.getAll(params)
          ...
          // data : complain 객체(백엔드에서 전송됨)
          .then(response => {
            // Todo : 아래 수정
            // 임시변수 complains, totalItems(서버의 결과데이터가 들어옴)
    ...
    // 클릭했을때 현재값을 저장함
    setActiveComplain(complain, index) {
